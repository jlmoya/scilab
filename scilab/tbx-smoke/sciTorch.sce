// STATUS: sciTorch's native gateway is NOT actually registered in this environment,
// so this smoke is expected to FAIL until the root cause below is fixed. Committed
// deliberately, same as tbx-smoke/scimax.sce: without it, the harness's delta>=1
// criterion alone false-PASSes sciTorch (its loader registers 1 macro library cleanly
// -- torch_load/torch_list/torch_unload etc. all exist as macros -- even though every
// one of its 8 native int_torch_* entry points is silently missing).
//
// Root cause, isolated by direct diagnostic (bypassing the swallowing catch below):
