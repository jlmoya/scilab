...
| B13 | **Three modules declare a JOGL dependency they never use.** `completion`, `graphic_objects` and `graphic_export` each carry JOGL/GlueGen in `pom.xml` while importing `com.jogamp` in **zero** source files. Only `scirenderer` (20 files), `gui` (9) and `renderer` (1) are real consumers. | `grep -rl com.jogamp modules/<m>/src/java` → 0 for all three; `grep -li jogl modules/*/pom.xml` → 6 modules | Inherited from the Ant build and carried across verbatim by the migration (*reproduce, don't improve*) — the POMs were transcribed from what Ant declared, not from what the code imports. | Drop the three dependencies. POM-only change, no source touched. Sequenced as Phase 1 of `docs/design/opengl-removal.md`, where it shrinks the dependency graph before any porting starts. |
