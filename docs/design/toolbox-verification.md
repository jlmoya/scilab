...
**The smoke is committed to this repo deliberately, precisely because of the previous paragraph:** without it, `delta=2` alone satisfies v1.1's `delta>=1 OR smoke OK` pass criterion and scimax would false-PASS on every clean checkout, even though the CAS handshake it exists to exercise never completes. Confirmed both ways: with the smoke present, `TBX_TIMEOUT=90 ./tbx-verify-all.sh scimax` returns `TIMEOUT` (reproduced independently at least three times now -- the final sweep, a follow-up controller probe, and this fix pass's own acceptance run); with the smoke temporarily moved aside -- i.e. exactly the state of a fresh clone before this fix -- the identical checkout returns `PASS` instead (the controller's probe). `tbx-smoke/scimax.sce` says so plainly in a status comment at its own top. Do not delete it, and do not "fix" it by weakening the check while `maxinit()` remains blocked -- a failing smoke here is the harness working as designed, not a bug in the smoke.

**A user decision (fix vs. delist) is pending.** scimax is the only toolbox in the 50-entry catalog that forks a live external CAS subprocess at runtime -- a materially different, ongoing maintenance burden (tied to whatever Maxima/SBCL ships next) than the rest of the verified set. Worth weighing against **sciSymPy**: it now genuinely carries the evidence this note used to merely assert. `tbx-smoke/sciSymPy.sce` (added in this same fix pass -- see `### sciSymPy` below) drives a real symbolic differentiation through the toolbox's Python/SymPy bridge over PIMS and passes (`smoke=OK`, confirmed via the harness and cross-checked manually outside it: `diff(x**2, x)` prints `2*x`). It covers symbolic-CAS use cases with none of scimax's subprocess-IPC fragility -- PIMS' Python bridge is an in-process embedded interpreter (`link()`-loaded, per `PIMS/pyenv/pyenv.sci`), not a forked CLI with a buffering handshake -- which is arguably a genuine supersession case for scimax, analogous to sciDatabase superseding scidb. Until the decision lands, the matrix row stays `TIMEOUT` and `cfg.verified` excludes `scimax`.
