...
# [3b] REMOVED — the @rpath system libs are now resolved natively. configure.ac adds the resolving
# rpaths (-Wl,-rpath,/usr/lib + the Homebrew gcc runtime dir) at link time, so libtool 2.5.4's
# @rpath install-names for libc++/libcurl/libgfortran/libquadmath load with no post-link fixup.
# (libjli stays resolved by the launcher's DYLD_LIBRARY_PATH.) See docs/design/build-modernization.md.
