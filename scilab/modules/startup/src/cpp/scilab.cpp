...
int main(int argc, char *argv[])
{
...
#endif
    {
        ...
        // -quit is HONOURED here, deliberately.
        //
        // This used to clear iForceQuit, on the reasoning that "quit will be
        // generated by the close of the pipeline". That holds for a pipe that
        // ends -- `echo cmd | scilab`, `< file scilab` -- but not for one that
        // is merely OPEN: a parent that hands the child a stdin pipe and never
        // writes to or closes it (java.lang.Runtime.exec does exactly this, and
        // so do several CI runners) delivers no EOF, ever. With -quit discarded
        // there was then nothing left to end the run, and Scilab blocked
        // forever in getPipeLine's fgets with its work already finished.
        //
        // Dropping the flag is safe because -quit has already been filtered
        // above ("ignore -quit if -e or -f are not given"), so reaching here
        // with it still set means the user gave BOTH a script AND a redirected
        // stdin. Their script is the work; stdin is incidental. Honouring the
        // flag they passed is the only reading that terminates.
        //
        // Ordering is deterministic, not racy: -e/-f is queued as a PRIORITARY
        // command and the -quit exit as a normal one, and GetCommand drains the
        // prioritary queue first, so the script always runs before the exit.
    }
