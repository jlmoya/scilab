...
// This is the Hilbert 5x5 matrix : A = hilbm(5);
A4 = [
...
M1 = C';
M2 = C;
[xcomputed, flag, err, iter, res] = conjgrad(A4, b4, "bicgstab", %M=M1, %M2=M2, tol=%eps, maxIter=11);
assert_checkalmostequal ( xcomputed , expected, 1.e8*%eps );
assert_checkequal ( flag , 0 );
assert_checktrue ( iter <= 10 ); // 9 under Windows with MKL / 10 under Linux with OpenBLAS
// Well-conditionned problem with a nonsymmetric matrix (top-right element)
